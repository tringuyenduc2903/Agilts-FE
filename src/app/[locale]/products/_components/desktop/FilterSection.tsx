...
function FilterSection({
  ...
  const renderedManufacturer = useMemo(() => {
    return (
      convertData(manufacturer?.data)?.map((m) => {
        return (
  ...
  const renderedVersions = useMemo(() => {
    return (
      convertData(versions?.data)?.map((d) => {
        return (
  ...
  return (
    <div
      className={`absolute bg-white top-[125%] -left-[100%] ${
        colors ? 'w-[70vw] 2xl:w-[75vw]' : 'w-[45vw] 2xl:w-[50vw]'
      } max-h-screen z-50 overflow-x-hidden overflow-y-auto ${
    ...
    >
      <div className='w-full h-full px-6 py-8 grid grid-cols-6 gap-x-8'>
        <div
          className={`${
            colors ? 'flex flex-col' : ' grid grid-cols-2 col-span-3'
          } gap-8`}
        >
        ...
        {minPrice.data?.raw && maxPrice.data?.raw && (
          <div className='col-span-2 flex flex-col gap-4'>
