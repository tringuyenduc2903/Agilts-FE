...
function Header() {
  ...
  return (
    <header className='absolute w-full z-10 p-8 flex justify-between items-center gap-8'>
        ...
        {routes?.map((r, index) => {
          ...
          return (
            <div className='relative' key={index}>
              ...
              >
                <button
                  className={`relative z-50 px-8 py-4 font-bold uppercase ${
                    pathname === r?.link || dropdown.includes(pathname)
                      ? 'border-neutral-200'
                      : 'border-neutral-50'
                  } border hover:bg-white ${
                    r?.name === isHoverButton && r?.isDropdown ? 'bg-white' : ''
                  ...
                  <p className='relative py-1'>
                    ...
                    <span
                      className={`absolute left-1/2 -translate-x-1/2 -bottom-1 ${
                        isHoverButton === r?.name ||
                        pathname === r?.link ||
                        dropdown.includes(pathname)
                          ? 'w-8'
                          : 'w-0'
                ...
                {/* `h-[${r?.dropdown?.length * r?.lineHeight + 64}px]` */}
                {r?.isDropdown && (
                  <div
      ...
      <div>
        <div className='relative w-[220px]'>
          <input
            className='w-full placeholder:text-neutral-700 focus:outline-none font-bold bg-transparent uppercase'
            type='text'
            placeholder='SEARCH'
            dir='rtl'
            ref={inputRef}
